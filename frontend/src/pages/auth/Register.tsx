...
export default function Register() {
  ...
  const finish = () => {
    setStatus("success");
    setTimeout(() => nav(`/${role ?? "student"}`), 2000);
  };
        ...
        <>
              ...
              {step === 1 && (
                <motion.div key="s1" custom={dir} initial={{ opacity: 0, x: dir * 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -dir * 40 }} transition={{ duration: 0.3 }} className="space-y-4">
                  <div>
                    ...
                    <input className={field} placeholder="Jordan Lee" />
                  </div>
                  <div>
                    <label className="mb-1.5 block text-sm font-medium text-ink">Email</label>
                    <input className={field} placeholder="you@university.edu" />
                  </div>
                  <div>
          ...
          <div className="mt-6 flex items-center justify-between gap-3">
            ...
            {step < 2 ? (
              ...
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={finish} className="inline-flex items-center gap-1.5 rounded-lg bg-primary px-5 py-2.5 text-sm font-semibold text-white shadow-card transition-all hover:bg-primary-dark">
